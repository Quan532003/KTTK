...
class TrainInfo:
    """
    Đại diện cho thông tin huấn luyện của một mô hình
    """
    def __init__(self, idInfo=None, epoch=None, learningRate=None, batchSize=None, 
        ...
        """
        Khởi tạo một đối tượng TrainInfo mới
        
        Args:
            idInfo (int, optional): ID của thông tin huấn luyện
            epoch (int, optional): Số epoch trong quá trình huấn luyện
            learningRate (float, optional): Tốc độ học
            batchSize (int, optional): Kích thước batch
            mae (float, optional): Mean Absolute Error
            mse (float, optional): Mean Squared Error
            trainingLosts (list, optional): Danh sách các TrainingLost
            accuracy (float, optional): Độ chính xác của mô hình
            timeTrain (str, optional): Thời gian huấn luyện
            trainDuration (int, optional): Thời lượng huấn luyện (giây)
        """
        self.idInfo = idInfo
        self.epoch = epoch
    ...
    def from_dict(cls, data):
        ...
        Returns:
            TrainInfo: Đối tượng TrainInfo mới
        """
        ...
        if training_losts_data:
            train_info.trainingLosts = [
                TrainingLost.from_dict(lost) if isinstance(lost, dict) else lost
